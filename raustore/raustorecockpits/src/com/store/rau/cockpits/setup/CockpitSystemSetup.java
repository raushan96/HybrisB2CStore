 ...
 */
package com.store.rau.cockpits.setup;
...
import com.store.rau.cockpits.constants.RaustoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = RaustoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(RaustoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(RaustoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						RaustoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/raustorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
