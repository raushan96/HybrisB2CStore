...
<form:form  action="${request.contextPath}/my-account/order/${order.code}/cancel/submit"
    ...
    <ul class="item__list cart__list cancel-order__list">
        ...
        <c:forEach items="${order.entries}" var="entry" varStatus="loop">
            <c:if test="${!entry.product.multidimensional && orderEntryCancelForm.cancelEntryQuantity[entry.entryNumber] > 0 }">
                ...
                <li class="item__list--item">
                    ...
                    <span class="qtyValue">
                        ...
                        <form:input class="form-control js-update-entry-quantity-input" type="text" size="1"
                                    path="cancelEntryQuantity[${key}]" readonly="true"/>
                    </span>
                    </div>
            ...
            <c:if test="${entry.product.multidimensional}">
                <c:forEach items="${entry.entries}" var="nentry" varStatus="nloop">
                    ...
                    <c:if test="${orderEntryCancelForm.cancelEntryQuantity[nentry.entryNumber] > 0}">
